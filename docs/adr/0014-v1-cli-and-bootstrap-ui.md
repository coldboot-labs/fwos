...
v1 ships an Appliance CLI and a basic UI, both talking to `netd` for Desired state. Neither is a Host program. The CLI is in `mgmt` and is also a client of the Host update program (ADR-0022). The v1 UI is not a Host update client. The UI starts in the Host netns and continues in `mgmt` after bootstrap (ADR-0028, ADR-0030).
