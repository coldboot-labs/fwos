...
Supersedes the NIC-placement clauses of ADR-0006 (Management NIC moves to `mgmt`; first boot leaves NICs in the Host netns until Bootstrap). The UI still never joins `fwd`. Amends ADR-0003: `mgmt` owns the UI, not a Management NIC. ADR-0052 supersedes this ADR's original assignment of NIC movement to `netd` and the empty-namespace-only restriction on the startup Host program.
