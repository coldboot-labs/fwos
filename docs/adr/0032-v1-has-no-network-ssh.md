...
v1 operators reach the appliance via the UI and Appliance console, not SSH. sshd is not a v1 product; reopen later. There is no injected-key Disk image: QEMU tests drive the Bootstrap console on serial and the UI over HTTPS. This drops sshd-in-`mgmt`, stick DNAT of port 22, Host-netns sshd, and “SSH works in mgmt after Bootstrap” as pass/fail.
